...
import styles from '../../styles/AboutUs/mobileAboutUs.module.css';
      ...
      <div style={{ backgroundColor: "#fff" }}>
         ...
         <div className={styles.leadership_section}>
            ...
            <div className={styles.leaders_grid}>
               {leaders?.map((ele, idx) => {
                     return (
                        <div key={ele?.id} className={styles.leaders_img_container} style={{ borderLeft: "1px solid #ddf8fd", borderRight: "1px solid #ddf8fd", borderBottom: idx === 9 ? "1px solid #ddf8fd" : "none", borderTop: "1px solid #ddf8fd" }}>
         ...
         <div className={styles.investor_section}>
            <div className={styles.section_heading}><h3>Championed by Visionaries</h3><div className={styles.heading_stroke} style={{ width: '12rem', margin: "auto" }}><Image src='/assets/industry/restaurant/features_hl 2.svg' alt='stroke' fill /></div></div>
            <div style={{ display: "flex", flexDirection: "column-reverse", justifyContent: "space-between" }}>
               ...
               <div className={styles.all_investors}>
                  {investorsArr?.map((ele, idx) => {
                     return (
                        <div key={ele?.id} style={{ padding: "3rem 2rem", display: 'flex', alignItems: "center", justifyContent: "center", borderTop: idx === 0 || idx === 1 || idx === 2 ? 'none' : "1px solid #ddf8fd", borderLeft: idx === 0 || idx === 3 ? 'none' : '1px solid #ddf8fd' }}>
                           <div className={styles.investor_images} style={{ height: ele?.Mheight, width: ele?.Mwidth }}><Image src={ele?.img} alt={ele?.altText} fill /></div>
                        </div>
                     )
                  })}
               </div>
