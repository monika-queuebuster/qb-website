...
import styles from '../../styles/cloudDashboard/MobileCloudDashboard.module.css';
...
const MobileCloudDashboard = () => {
  const { commonMarqueTitle } = content
  const { testimonialHeadText } = content.testimonial
  const { giveitTryHeading, giveitTryDesc } = content.retail
  const { CDPageHeading, CDTypingText1, CDTypingText2, CDPageDesc, industriesWeServe, businessGrows10X, enhancedTransperency, lossPrevention, CDFAQHeading, otherHighlights } = content.features
  const router = Router;
    ...
    <>
        ...
        <div className={styles.left_container}>
          ...
          <div className={styles.right_container}>
            <div className={styles.section1_image_container}>
              <Image src='https://quebuster.s3.ap-south-1.amazonaws.com/website/assets/dashboard/cloud-dashboard-hero-img.webp' alt='estore hero image' fill />
            </div>
          </div>
      ...
      <div className={styles.section_four}>
        <div style={{ display: 'flex', justifyContent: 'end' }}><div className={styles.section4_image_container}>
          <Image src={CDBanner?.image} alt={CDBanner?.altText} fill />
        </div>
        </div>
        <div style={{ padding: '3rem 2rem' }}>
          <div className={styles.section4_left_side}>
            <h4 className={styles.section_four_heading}>{CDBanner?.title}</h4>
            <p className={styles.section_four_content}>{CDBanner?.content}</p>
          </div>
      ...
      <div className={styles.section_five}>
        <div className={styles.section5_container}>
          ...
          <div className={styles.section5_right_container}>
            <div><h3 className={styles.rewarding_section_heading}>{businessGrows10X}</h3><div className={styles.business_stroke_img}><Image src='/assets/strokes_svg/business_hl.svg' alt='business stroke' fill /></div></div>

            <div style={{ display: 'flex', gap: '3rem', margin: '2rem 0 3.75rem 0', width: '100%', justifyContent: 'space-between', flexDirection: 'column' }}>
              <div>
                <p className={styles.LC_number}>75%</p>
                <p className={styles.LC_heading}>{enhancedTransperency}</p>
              </div>
              <div>
                <p className={styles.LC_number}>98%</p>
                <p className={styles.LC_heading}>{lossPrevention}</p>
              </div>
            </div>
      ...
      <div>
        <FAQmobile FaqTableData={CDFAQ} width={"auto"} heading={CDFAQHeading} />
      </div>
...
export default MobileCloudDashboard