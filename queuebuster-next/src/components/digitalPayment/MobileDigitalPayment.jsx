...
import styles from '../../styles/digitalPayments/MobileDigitalPayment.module.css';
...
import { payments, paymentFAQ, paymentBanner, paymentHighlights } from '../../constants/features'
...
const MobileDigitalPayment = () => {
  const { commonMarqueTitle } = content
  const { testimonialHeadText } = content.testimonial
  const { giveitTryHeading, giveitTryDesc } = content.retail
  const { paymentHeading, paymentTypingText1, paymentTypingText2, paymentTypingText3, paymentPageDesc, indiaWorldLeader, worthOfTransactions, yoyGrowth, paymentFAQHeading } = content.features
    ...
    <>
      ...
      <div className={styles.section_one}>
        <div className={styles.left_container}>
          <h1>
            <span className={styles.black_text}>{paymentHeading}</span><br />
            {/* <span className={styles.blue_text}><ReactTyped strings={[paymentTypingText1, paymentTypingText2, paymentTypingText3]} typeSpeed={200} loop /></span> */}
          </h1>
          <div className={styles.right_container}>
            <div className={styles.section1_image_container}>
              <Image src='https://quebuster.s3.ap-south-1.amazonaws.com/website/assets/payment/payments-hero-img.webp' alt='estore hero image' fill />
            </div>
          </div>
          <p className={styles.page_subheading}>{paymentPageDesc}</p>
      ...
      <div className={styles.section_four}>
        <div style={{ display: 'flex', justifyContent: 'end' }}><div className={styles.section4_image_container}>
          <Image src={paymentBanner.image} alt={paymentBanner.altText} fill />
        </div>
        </div>
        <div style={{ padding: '3rem 2rem' }}>
          <div className={styles.section4_left_side}>
            <h4 className={styles.section_four_heading}>{paymentBanner.title}</h4>
            <p className={styles.section_four_content}>{paymentBanner.content}</p>
          </div>
      ...
      <div className={styles.section_five}>
        <div className={styles.section5_container}>
          ...
          <div className={styles.section5_right_container}>
            <div><h3 className={styles.rewarding_section_heading}>{indiaWorldLeader}</h3><div className={styles.business_stroke_img}><Image src='/assets/strokes_svg/business_hl.svg' alt='business stroke' fill /></div></div>

            <div style={{ display: 'flex', gap: '3rem', margin: '2rem 0 3.75rem 0', width: '100%', justifyContent: 'space-between', flexDirection: 'column' }}>
              <div>
                <p className={styles.LC_number}>$1.5 Trillion</p>
                <p className={styles.LC_heading}>{worthOfTransactions}</p>
              </div>
              <div>
                <p className={styles.LC_number}>13.75 %</p>
                <p className={styles.LC_heading}>{yoyGrowth}</p>
              </div>
            </div>
      ...
      <div className={styles.mobile_industries_section}>
        <div style={{ width: "fit-content", margin: "auto" }}><h3 className={styles.industry_heading}>Industries we serve</h3> <div className={styles.industries_stroke_img}><Image src='/assets/strokes_svg/industries_hl.svg' alt='industries' fill /></div></div>
      ...
      <div>
        <FAQmobile FaqTableData={paymentFAQ} width={"auto"} heading={paymentFAQHeading} />
      </div>
...
export default MobileDigitalPayment